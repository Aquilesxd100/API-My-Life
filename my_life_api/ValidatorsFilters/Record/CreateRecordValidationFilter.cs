    ...
    public class CreateRecordValidationFilter : ICustomActionFilter
    {
            ...
            ActionExecutionDelegate next)
        {
            ...
            if (string.IsNullOrEmpty(record.ano) || record.ano.Trim().Length == 0) {
                throw new CustomException(400, "O ano do registro é obrigatório e não pode ficar vazio.");
            }
            ...
            if (string.IsNullOrEmpty(record.conteudo) || record.conteudo.Trim().Length == 0) {
                throw new CustomException(400, "O conteúdo do registro é obrigatório e não pode ficar vazio.");
            }

            if (record.conteudo.Length > 3500) {
                throw new CustomException(400, "O conteúdo do registro deve ter no máximo 3500 caracteres.");
            }

            if (Validator.HasInvalidCharacters(record.conteudo)) {
                throw new CustomException(400, "O conteúdo do registro contém caracteres inválidos.");
            }
